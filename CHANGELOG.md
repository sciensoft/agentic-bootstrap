...
- **Agent-agnostic mechanical enforcement for the trigger index** — the design in [`AGENTIC-B.Improvements.md`](./AGENTIC-B.Improvements.md) §9, ported into the generator (Cut 2 of the context-management improvement series). Three layers, universal by default and per-host opt-in:
  - **`make rules-check`** — new target in every `Makefile` variant (Python, TypeScript/Node, Go, Rust, fallback). Three checks: orphaned rule files (present on disk but not indexed in `AGENTS.md`), broken links (path in `AGENTS.md` doesn't resolve on disk), and the byte/token budget for the always-loaded pair. Runs by habit or in CI — a rule that stops applying because nothing points at it any more is what §6.4 warns about.
...
- **Self-application to this repo.** `Makefile` picks up the `rules-check` target; `.agents/rules/workflow.md` picks up the `## Consulted rules` schema. Other pieces skipped intentionally — this repo has no UI/FRONTEND/METRICS paths (empty case ladder would be dead code), no `.pre-commit-config.yaml` yet (separate concern), no Cursor adapter to reinforce.
