...
- **Landing-page rebuild for the context-management overhaul.** `docs/index.html` gains four coordinated updates:
  - **Per-agent tabs** on the hero CLI mockup and the *A repo that knows its own rules* file-tree section. Seven agents (Claude Code · Cursor · Codex CLI · Windsurf · Copilot · Aider · OpenCode); Claude Code default; tab state shared via a small JS block so clicking a tab in either place switches both. Fixes the previous kitchen-sink mockup that showed every adapter file at once — misleading for readers who assumed they'd all land on any single-agent pick.
  - **Runtime footprint section refreshed** with the new numbers (~4.5k tokens always-loaded, measured — down from ~47k in the pre-Cut-1 shape) and two new subsections: *The trigger index* (reactive vs proactive-discipline flavors) and *Mechanical enforcement (opt-in, per host)* (Claude `PreToolUse` hook, Cursor `alwaysApply` rule, agent-agnostic pre-commit hook). The "Every session after" card is highlighted with an accent border to draw the eye to the 10× improvement.
  - **Fixed-rate-subscription banner** at the top of the page. Names the plans that absorb scaffold cost (Claude Code, Cursor Pro, Codex on ChatGPT, Copilot) and points at the *Runtime footprint* section for the actual numbers. Not a functionality gate — a cost heads-up for anyone on per-token API billing.
  - **`Continue.dev` removed** from meta description, hero *Works-with* strip, *How it works* copy, and *Permission postures* paragraph. Product discontinued (team joined Cursor); leaving it in the page implied support that no longer exists.
