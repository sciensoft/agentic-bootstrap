...
| **R2b** | ✅ | *Origin story* section — *"Built from real engineering"*. Positioned after the 60-second demo and before *What you get*, so the reader hits it at the *"who built this and why?"* moment right after seeing the mechanism. Full section (`py-24`, `max-w-7xl` container / `max-w-3xl` inner reading width, H2 at `text-4xl md:text-5xl` — matches neighbouring section H2s). Personal-voice narrative arc: *build a project, engineering grows organically, start a second project, "I wish this project had everything I built into the last one" (blockquote), by the third project it's obvious → that's why AGENTIC_BOOTSTRAP exists.* Closes with a five-line rhythmic action stanza (*Start with the bootstrap. / Keep what works. / Remove what doesn't. / Add your own skills, workflows, standards, and conventions. / Evolve it with your project.*) and a two-line positioning closer (*AGENTIC_BOOTSTRAP gives you the first version of your operating model. / What it becomes is yours.*) with gradient on the brand name. **Not in the original ADR-0010 R2-R7 plan; added on user request as R2b.** ADR-0010's Decision-body section list should be extended to acknowledge this insertion when the ADR-body sweep lands (currently deferred). | additive (~250 words, prose-narrative full section) | §origin (new) |
...
8. **Built from real engineering** — origin story; personal-voice narrative between demo and *What you get* on the current page. Not in the original R2-R7 plan. *(R2b ✅)*
