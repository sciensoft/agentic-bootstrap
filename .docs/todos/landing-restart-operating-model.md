...
| **R2** | ✅ | Compressed delivery: added a single *"Why this matters?"* Aha section between the hero and the 60-second demo (chip-row shape: eyebrow + one-sentence framing + four knowledge-type chips + three-line resolution with gradient accent on *AGENTIC_BOOTSTRAP*). The planned §2 *What just changed* + §3 *The new bottleneck* split into ~250 words is retired in favour of ~50 words in one beat — the demo section that follows carries the concrete affordance, so a longer body-prose interstitial would compete with it. | additive (~50 words, chip-row visual) | §2 (merged §3) |
