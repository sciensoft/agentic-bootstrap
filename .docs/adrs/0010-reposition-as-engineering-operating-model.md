...
- **H1:** *Software · engineering · changed.* (each word on its own line, staircase rhythm) → *So does the discipline.* (line 4, `text-gradient`). The four-line stack creates dramatic pacing on the category-shift assertion; the fourth line *resolves it positively* — engineering changed, **and so has the discipline that runs it** (the rigor/operating model the bootstrap installs). The elided verb in *"so does"* mirrors *"changed"* for a parallel rhythm. Two prior drafts were withdrawn after user feedback: *"Repositories haven't"* (put the *repository* in the protagonist role when the protagonist is the human–AI collaboration; see *Protagonist convention* in Consequences); *"You're losing most of it"* (correct on protagonist but framed the line as the gap rather than as what the bootstrap delivers).
