...
- **Eyebrow:** *An engineering operating model for human–AI software development*
- **H1:** *Software engineering changed. Repositories haven't.*
- **Lede:** *Agentic AI has made engineering knowledge abundant. AGENTIC_BOOTSTRAP makes it durable.*

The hero's only job is to create curiosity. It does not explain features, list artifacts, or describe mechanism. The next-section anchor and the philosophical body sections (added in subsequent commits R2–R7) carry the explanation.
...
- **Hero shrinks to text-only.** The right-column terminal screenshot (showing the interview Q&A and resulting file tree) is removed — it is the most direct violation of "no implementation in the hero." A future hero illustration that *creates* curiosity rather than *explains* mechanism may go back into that column; until then, whitespace and the radial glow carry the visual weight.
- **Auxiliary hero chrome stays transitionally.** The four checkmarks (Project-agnostic · Re-runnable · No dependencies · One file) and the eight-tool *Works with* strip are not philosophy and don't fit the new hero, but deleting them in R1 would leave a product-information gap until R6 re-stages them in dedicated bands below the philosophical sections. They stay as transitional chrome and migrate cleanly in R6.
