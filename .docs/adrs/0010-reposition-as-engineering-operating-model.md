...
- **H1:** *Software · engineering · changed.* (each word on its own line, staircase rhythm) → *The discipline runs at AI speed.* (line 4, `text-gradient`). The four-line stack creates dramatic pacing on the category-shift assertion; the fourth line names what the bootstrap delivers — *the same discipline that always existed, now applied at the pace AI is producing engineering*. Discipline didn't change; its delivery did. H1 type size sits one tier below the standard (`text-4xl md:text-5xl lg:text-6xl`, was `text-5xl md:text-6xl lg:text-7xl`) so the longer line-4 sentence fits on a single line at every breakpoint inside the `lg:col-span-7` container without wrapping. Three prior drafts were withdrawn after user feedback: *"Repositories haven't"* (put the *repository* in the protagonist role when the protagonist is the human–AI collaboration; see *Protagonist convention* in Consequences); *"You're losing most of it"* (correct on protagonist but framed the line as the gap rather than as what the bootstrap delivers); *"So does the discipline"* (positive and parallel-rhythm correct, but implied the discipline itself changed — it didn't; it just couldn't be applied consistently at pre-AI manual speed, and what changed is that the bootstrap makes it run at AI's speed).
...
- **Subhead:** *One markdown file. Any agentic coding assistant. One prompt. Your repo grows the discipline that holds every session's engineering — architectural decisions, the reasoning behind them, deferred ideas, conventions, security context — so the next session walks in with all of it. Tool-agnostic. Cross-stack. Re-runnable.* — restores the v1 subhead's active *"Your repo grows the discipline"* central clause (the repo is accepted as a supporting-actor here, see *Protagonist convention*), with substance rebuilt under the new positioning: the discipline is now framed as *what holds engineering across sessions*, the inserted list is categories of knowledge rather than file artifacts, the closing payoff (*"the next session walks in with all of it"*) names the inheritance loop. An earlier supporting-copy draft used *"engineering memory"* in place of *"discipline"*; reverted to *"discipline"* for unity with the H1's line 4.
