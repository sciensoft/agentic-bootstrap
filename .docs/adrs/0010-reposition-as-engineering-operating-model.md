...
- **Subhead:** *The fastest engineering conversations now happen between humans and AI. AGENTIC_BOOTSTRAP makes sure the architecture, decisions, trade-offs, and future ideas survive long after the session ends.* — the *R1 initial* lede and long-form subhead collapse into a single two-sentence paragraph. First sentence names what changed (the collaboration itself has become the fastest engineering conversation — the category shift humanised, without saying *"faster than solo work"* which reads as marketing). Second sentence names what the product does about it (durability of the four categories of engineering knowledge most at risk: architecture, decisions, trade-offs, future ideas — which map onto ADRs, prompt files, ADR consequences sections, and `.docs/todos/` respectively, though no file names appear on the hero). *Idempotent · cross-stack · tool-agnostic* and the *day-one discipline* file-list vignette both drop out of the hero at this iteration; they migrate downstream — *tool-agnostic* to R6's Works-with band, *day-one discipline* to R5's substrate framing — where the concrete anchoring lands with more room. Emphasis weights: `text-zinc-100 font-medium` on *the fastest engineering conversations now happen between humans and AI* (primary framing); `text-gradient font-semibold` on *AGENTIC_BOOTSTRAP* (product-name visual anchor); `text-zinc-100 font-medium` on *survive long after the session ends* (mirrors the *abundant / durable* contrast weighting from the withdrawn *R1 initial* lede).
