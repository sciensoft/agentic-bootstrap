...
- **H1:** two lines, `text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight leading-[1.05]` (v1 sizing restored — the smaller `text-4xl md:text-5xl lg:text-6xl` tier from the *R1 initial* draft was needed for the 5-line staircase but is now too small for a 2-line pair):
  ```
  Keep the flow.
  Keep the engineering.
  ```
  Imperative parallel pair — the reader is the implicit subject of both verbs, honouring the *Protagonist convention*. *Flow* names the natural affordance of human–AI collaboration (the pace, the conversational tempo, the fact that engineering happens as talk); *engineering* names the discipline that has to survive it (decisions, architecture, trade-offs, retained rationale). Gradient is applied to *engineering* alone (the second-half differentiator word), echoing v1's underline-on-*Agent* pattern of spotlighting a single identity word without decorating the whole line.
